...
import { buildFilters, emptyRule, looksLikeNpi, ruleIsSet } from './manifest.js';
...
f = buildFilters(file({ trx: { ...emptyRule('number'), min: '1' } }, { trx: 'scripts' }));
...
f = buildFilters(file({ npi: { ...emptyRule('string'), luhn: true } }));
...
f = buildFilters(file({ region: { ...emptyRule('string'), luhn: true } }));
t('luhn NOT sent for a non-NPI column', f.length === 0, f);
f = buildFilters(file({ npi: { ...emptyRule('string'), luhn: true } }, { npi: 'region' }));
t('renaming npi -> region still allows it (original name matches)', f.length === 1, f);

console.log('\n7. not_null is independent of type');
f = buildFilters(file({ region: { ...emptyRule('string'), notNull: true } }));
t('not_null sent alone', f.length === 1 && f[0].type === 'not_null', f);
f = buildFilters(file({ trx: { ...emptyRule('number'), notNull: true, min: '5' } }));
...
f = buildFilters(file({
  trx: { ...emptyRule('number'), min: '1' },
  region: { ...emptyRule('string'), values: ['East'] },
  month: { ...emptyRule('date'), start: '2026-01-01' },
}));
