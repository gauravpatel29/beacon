        ...
        if (result && result.files && result.files.length > 0) {
          const newEntries = result.files.map((rf, idx) => {
            ...
            if (lowerName.includes("sale") || lowerName.includes("trx") || lowerName.includes("nrx")) {
              suggestedCat = "sales";
            } else if (lowerName.includes("call") || lowerName.includes("sample") || lowerName.includes("hcp") || lowerName.includes("rep")) {
              suggestedCat = "hcp_promo";
            } else if (lowerName.includes("tv") || lowerName.includes("dma") && lowerName.includes("spend")) {
              suggestedCat = "dma_promo";
            } else if (lowerName.includes("map") || lowerName.includes("bridge") || lowerName.includes("crosswalk")) {
          ...
          <div className="bg-slate-900 text-white rounded-2xl p-5 flex items-center justify-between flex-wrap gap-4 shadow-xl">
            <div>
              ...
              <p className="text-[11px] text-slate-400 mt-0.5">
                Ready to proceed to Exploratory Data Analysis
              </p>
            </div>
...
function StandardizePanel({ activeFile, onUpdated }) {
  ...
  return (
    <div className="space-y-6">
      {/* Schema & Data Type Modifier Table */}
      <div className="space-y-2">
        <div className="flex justify-between items-center">
...
// ─── Sub-Panel: Modify Granularity (HCP × Period Aggregation) ─────────────────
function GranularityPanel({ activeFile, onUpdated }) {
  ...
  const handleApplyGranularity = async () => {
    ...
    try {
      ...
      toast.success("Granularity modified successfully (1 row per HCP per period)");
    } catch (err) {
  ...
  return (
    <div className="space-y-4">
      ...
      <MultiSelect label="Numerical Columns to Aggregate (Summed per HCP/Geo per period)" value={numCols} onChange={setNumCols} options={activeFile.columns} />
      {numCols.map((col) => (
