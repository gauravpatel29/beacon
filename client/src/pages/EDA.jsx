...
export default function EDA() {
  ...
  // ─── 1. ARD Registry Selector ─────────────────────────────────────────────
  const savedArds = useMemo(() => state.savedArds || [], [state.savedArds]);
  const [selectedArdId, setSelectedArdId] = useState(() => {
    return savedArds.length > 0 ? savedArds[savedArds.length - 1].id : "active_granular";
  });

  const activeCsv = useMemo(() => {
    if (savedArds.length > 0 && selectedArdId !== "active_granular") {
      const match = savedArds.find((a) => a.id === selectedArdId);
      if (match) return match.csv_data;
    }
    return state.granularCsvData || state.filteredCsvData || state.mergedCsvData;
  }, [savedArds, selectedArdId, state]);

  // Keep backup for restoring exclusions
