...
def _parse_csv_resilient(content: str) -> pd.DataFrame:
    ...
    clean_content = str(content).strip().lstrip("\ufeff")
    first_line = clean_content.split("\n")[0] if "\n" in clean_content else clean_content
    delims = [",", ";", "\t", "|"]
    counts = {d: first_line.count(d) for d in delims}
    chosen_delim = max(counts, key=counts.get) if max(counts.values()) > 0 else ","

    try:
        df = pd.read_csv(io.StringIO(clean_content), sep=chosen_delim, low_memory=False, on_bad_lines="skip")
    except Exception:
        df = pd.read_csv(io.BytesIO(clean_content.encode("latin-1")), sep=chosen_delim, low_memory=False, on_bad_lines="skip")
...
def _find_col_case_insensitive(df: pd.DataFrame, target_col: str) -> Optional[str]:
...
def _clean_key_list(key_input) -> List[str]:
    if isinstance(key_input, list):
        return [str(k).strip() for k in key_input if str(k).strip()]
    if isinstance(key_input, str):
        return [k.strip() for k in key_input.split(",") if k.strip()]
    return []


def _standardize_column(series: pd.Series, is_date: bool = False) -> pd.Series:
    s_clean = series.astype(str).str.strip().str.replace(r"\.0$", "", regex=True)
    s_clean = s_clean.replace({"nan": "", "None": "", "null": "", "<NA>": "", "NaN": ""})
    if is_date:
        parsed = pd.to_datetime(s_clean, errors="coerce", dayfirst=True)
        if parsed.isna().sum() > 0:
            parsed_d0 = pd.to_datetime(s_clean, errors="coerce", dayfirst=False)
...
def execute_ard_pipeline(
    ...
    if not dfs:
        raise ValueError("Uploaded datasets are empty or contain 0 rows.")

    dataset_registry = {name: df.copy() for name, df in dfs.items()}
    current_df = None
    lineage_sources = []
    ...
    for idx, step in enumerate(steps):
        ...
        right_df = dfs.get(r_name)
        if right_df is None:
            right_df = dfs.get(r_name.lower())

        if left_df is None or left_df.empty:
            raise ValueError(f"Step {idx + 1}: Left dataset '{l_name}' not found. Available: {list(dfs.keys())}")
        if right_df is None or right_df.empty:
            raise ValueError(f"Step {idx + 1}: Right dataset '{r_name}' not found. Available: {list(dfs.keys())}")
